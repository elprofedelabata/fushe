...
import { convertirXrhoAFushe } from "../penalara/convertir";

function mostrarUso(): never {
  process.stderr.write(
    "Uso: npm run convertir:penalara -- <entrada.xrho> <salida.fushe> [--anonimizar] [--sobrescribir]\n",
  );
  process.exit(2);
}
...
function resumen(documento: DocumentoFushe): Record<string, unknown> {
  ...
  const tipoPorActividad = new Map(documento.actividades.map((a) => [a.id, a.tipo]));
  ...
  return {
    ...
    perfiles: documento.perfiles.map((perfil) => ({
      id: perfil.id,
      tramos: perfil.tramos.length,
    })),
...
function principal(): void {
  ...
  let documento = convertirXrhoAFushe(readFileSync(entrada), { nombreArchivo: entrada });
